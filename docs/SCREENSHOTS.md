# Screenshot Tour

This page shows the main desktop surfaces in Filament Manager. The smaller
preview strip in the README links here for a fuller product walkthrough.

Status note, 2026-07-01: these screenshots are still useful for orientation, but
minor UI details may differ from the latest app while the interface continues to
evolve.

<p align="center">
  <a href="#dashboard"><img src="screenshots/dashboard-thumb.jpg" alt="Dashboard" width="150"></a>
  <a href="#inventory"><img src="screenshots/inventory-thumb.jpg" alt="Inventory" width="150"></a>
  <a href="#loans"><img src="screenshots/loans-thumb.jpg" alt="Loans" width="150"></a>
  <a href="#printers"><img src="screenshots/printers-thumb.jpg" alt="Printers" width="150"></a>
  <a href="#statistics"><img src="screenshots/statistics-thumb.jpg" alt="Statistics" width="150"></a>
  <a href="#add-filament"><img src="screenshots/add-filament-thumb.jpg" alt="Add filament" width="150"></a>
  <a href="#filament-details"><img src="screenshots/filament-details-thumb.jpg" alt="Filament details" width="150"></a>
</p>

## Dashboard

Inventory health, printer activity, low-stock signals, recent activity and
library/webapp status in one overview.

![Dashboard overview](screenshots/dashboard.jpg)

## Inventory

Searchable spool cards with remaining weight, location, material/vendor badges
and low-stock state.

![Inventory grid](screenshots/inventory.jpg)

## Loans

Outgoing loans, borrowed-in rolls and return handling in the same history view.

![Loan history](screenshots/loans.jpg)

## Printers

Printer and AMS slot state with live Bambu observations, current assignments,
weight updates and candidate matching.

![Printer slot overview](screenshots/printers.jpg)

## Statistics

Material use, printer activity, ownership split and loan consumption.

![Statistics overview](screenshots/statistics.jpg)

## Add Filament

Catalog-backed stock entry for Bambu, eSUN and generic/manual rolls, with
wishlist/order workflow and Bambu box-code batch entry.

![Add filament flow](screenshots/add-filament.jpg)

## Filament Details

Roll-level maintenance for weight, tare, home location, ownership, QR companion
links, RFID and lost-status handling.

![Filament detail panel](screenshots/filament-details.jpg)
