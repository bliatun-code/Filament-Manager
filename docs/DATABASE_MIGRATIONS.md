...
- `STRUCTURAL_MIGRATIONS` in
  ...
  versioned transitions. It currently contains `003`, the schema 1 to schema 2
  transition.
...
Assume the manifest ends at sequence `003` and schema version 2:

1. Add `src/database/migrations/004_short_description.sql`. Use the next
...
2. Append one `StructuralMigration` row with `from_version: 2` and
   `to_version: 3`. Never insert or reorder rows in the existing sequence.
...
4. Append the matching manifest entry, set `currentSchemaVersion` to 3, and
