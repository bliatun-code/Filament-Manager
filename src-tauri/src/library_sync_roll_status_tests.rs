...
fn request(host: &SyntheticHost, generation: u64) -> LibrarySyncInventoryBulkMutationInput {
    ...
    serde_json::from_value(serde_json::json!({
        ...
        "mutation":{"action":"ROLL_STATUS","target_status":"LOST","expected_slot_id":"printer / one_ams_1_slot_1",
...
#[test]
fn roll_status_host_commits_once_and_preserves_client_rows_and_host_weight() {
    ...
    assert_eq!(changed.status, "LOST");
    assert_eq!(changed.current_weight_g, spool.current_weight_g);
    assert_eq!(changed.remaining_g, spool.remaining_g);
    assert_eq!(changed.location_id, spool.home_location_id);
...
#[test]
fn roll_status_old_host_bad_generation_unpaired_and_health_transition_send_no_post() {
    let _serial = NETWORK_TEST_LOCK
        .lock()
        .unwrap_or_else(|error| error.into_inner());
    for case in [
        ...
        if case == "legacy" {
            assert!(error.contains("inventory.roll_status.host_unsupported"));
        }
...
#[test]
fn roll_status_host_rejects_changed_slot_and_late_failure_without_partial_changes() {
    let _serial = NETWORK_TEST_LOCK
        .lock()
        .unwrap_or_else(|error| error.into_inner());
    for stale in [false, true] {
        ...
        if stale {
            ...
            FilamentDatabase::open(&host.db_path).unwrap().connection().execute_batch("CREATE TRIGGER fail_status BEFORE INSERT ON spool_history_events WHEN NEW.event_type='STATUS_UPDATED' BEGIN SELECT RAISE(ABORT,'late status failure'); END;").unwrap();
        }
        let before_client = snapshot(&state.db_path);
        let before_host = snapshot(host.db_path.to_str().unwrap());
...
#[test]
fn roll_status_acknowledged_write_survives_cache_target_change_and_rejects_invalid_receipts() {
    let _serial = NETWORK_TEST_LOCK
        .lock()
        .unwrap_or_else(|error| error.into_inner());
    for receipt in [
