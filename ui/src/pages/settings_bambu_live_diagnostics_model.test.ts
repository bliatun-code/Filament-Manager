...
test("Bambu live diagnostic tray card infers unknown RFID material from filament name", () => {
  const card = buildSettingsBambuLiveDiagnosticTrayCard({
    amsReadInProgress: false,
    capturedTraySnapshot: null,
    spoolRows: [
...
test("Bambu live diagnostic tray card infers unknown RFID material from tray preset name", () => {
  const card = buildSettingsBambuLiveDiagnosticTrayCard({
    amsReadInProgress: false,
    capturedTraySnapshot: null,
    spoolRows: [
...
test("Bambu live diagnostic tray card falls back to catalog when saved-RFID metadata is skipped", () => {
  const card = buildSettingsBambuLiveDiagnosticTrayCard({
    amsReadInProgress: false,
    catalogRows: [
      createCatalogRow({
  ...
  assert.equal(card.showCandidateCards, true);
  assert.equal(card.matchLabel, "PLA Matte · Black");
  assert.deepEqual(card.candidates.map((candidate) => candidate.key), ["bambu-matte-black"]);
  assert.equal(
...
test("Bambu live diagnostic tray card matches composite swatches for unknown RFID", () => {
  const card = buildSettingsBambuLiveDiagnosticTrayCard({
    amsReadInProgress: false,
    capturedTraySnapshot: null,
    spoolRows: [
  ...
  assert.equal(card.matchKind, "metadata_single");
  assert.equal(card.matchLabel, "PLA Matte · Black/White");
  assert.deepEqual(card.candidates.map((candidate) => candidate.key), [
    "multi-swatch-bambu",
  ]);
});
