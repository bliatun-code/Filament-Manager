...
> &
  ...
  const titleId = "inventory-bulk-review-title";
  return (
    <div
...
export function InventoryBulkActionsPanelView({
  ...
  return (
    <section
      className="surface-subtle space-y-4 p-4"
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-600 dark:text-slate-300" aria-live="polite">
            {selectedCount > 0 ? copy.selected(selectedCount) : copy.noSelection}
          </p>
      ...
      {review ? (
        ...
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
          <fieldset className="rounded-xl border border-slate-200 p-3 dark:border-slate-700">
            ...
            <label className="mt-1 block text-xs font-semibold text-slate-700 dark:text-slate-200">
              ...
              >
                ...
                {locationTargets.map((location) => (
                  <option key={location.id} value={location.id} disabled={location.archived}>
                    {location.archived
            ...
            <button
              type="button"
              className={`mt-3 ${modalActionButtonClassName("primary")}`}
          ...
          <fieldset className="rounded-xl border border-slate-200 p-3 dark:border-slate-700">
            ...
            <button
              ...
              onClick={() => onRequestStatusReview(statusTarget)}
            >
              {copy.reviewStatus}
            </button>
          </fieldset>
