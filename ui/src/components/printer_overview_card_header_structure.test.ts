...
test("printer overview header lets long names and metadata wrap inside compact cards", () => {
  ...
  assert.match(
    headerSource,
    /order-3 grid w-full grid-cols-4 gap-2 min-\[1080px\]:order-2 min-\[1080px\]:w-auto min-\[1080px\]:min-w-\[18rem\]/,
  );
  assert.match(
    headerSource,
    /order-2 w-full min-\[640px\]:-mt-8 min-\[640px\]:pl-\[10\.25rem\] min-\[1080px\]:order-3/,
  );
  assert.doesNotMatch(headerSource, /min-\[900px\]:(?:order-2|order-3|w-auto|min-w-\[18rem\])/);
