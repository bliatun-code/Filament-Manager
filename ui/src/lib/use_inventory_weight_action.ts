...
import { shouldReactivateSpoolFromMeasuredTotal } from "./inventory_spool_detail_actions_model";
import { executeInventoryBulkMutation, executeLibrarySyncHostInventoryBulkMutation,
  type InventoryRollWeightCommand } from "./tauri_inventory_client";
...
export function useInventoryWeightAction(input: Input) {
  ...
  const snapshotKey = JSON.stringify([selectedSpool?.status, selectedSpool?.locationId,
    selectedSpool?.homeLocationId, selectedSpool?.remainingGrams, selectedSpool?.currentWeightGrams, input.resolvedTare,
  ...
  const currentScope = useRef<{ view: typeof view; busy: boolean; committedSnapshot: object | null; committedTotal: number | null } | null>(null);
  const currentSnapshot = useRef(snapshot);
  const [weightError, setWeightError] = useState<string | null>(null);
  useLayoutEffect(() => { currentSnapshot.current = snapshot; }, [snapshot]);
  useLayoutEffect(() => {
    const scope = { view, busy: false, committedSnapshot: null as object | null, committedTotal: null as number | null };
  ...
  useLayoutEffect(() => {
    if (input.manageBusy && !currentScope.current?.busy) setWeightError(null);
  }, [input.manageBusy]);

  async function submitWeight(grams: number) {
    ...
    if (!scope || !selectedSpool || !input.tauriAvailable || input.manageBusy || !isCurrent() || scope.busy ||
      currentSnapshot.current !== snapshot || (scope.committedSnapshot === snapshot && scope.committedTotal === Math.max(0, Math.round(grams)))) return;
    if (input.clientReadOnly ? !input.canUseClientHostWrite() : !input.ensureLocalWriteAllowed()) return;
    const { t } = input;
    if (input.loanedOut) {
      setWeightError(t("errors.loanedSpoolEditBlocked", "Return the active outbound loan before changing status, location, or ownership for this roll."));
      return;
    }
    const measuredTotal = Math.max(0, Math.round(grams));
    if (!Number.isFinite(grams) || !Number.isSafeInteger(measuredTotal)) {
      setWeightError(t("inventory.error.invalidWeight", "Weight value is invalid."));
      return;
    }
    const reactivate = shouldReactivateSpoolFromMeasuredTotal(selectedSpool.status, measuredTotal, input.resolvedTare);
    const command: InventoryRollWeightCommand = {
      action: "ROLL_WEIGHT", measured_total_g: measuredTotal,
      expected_remaining_g: selectedSpool.remainingGrams ?? null, expected_tare_g: input.resolvedTare,
    ...
    try {
      try {
        ...
        if (isCurrent()) setWeightError(commandErrorText(error,
          t("inventory.error.updateWeight", "Failed to update weight."), t));
        return;
      }
      if (!isCurrent()) return;
      scope.committedSnapshot = snapshot;
      scope.committedTotal = measuredTotal;
      if (reactivate) input.setInfoMessage(t("inventory.refilledAuto", "Roll reactivated from new measured weight."));
      else if (input.clientReadOnly) input.setInfoMessage(t("inventory.clientWeightUpdated", "Weight updated on the host library."));
      let failed = false;
      ...
      if (isCurrent() && (failed || results.some(result => result.status === "rejected"))) {
        setWeightError(t("inventory.error.loadInventory", "Failed to load inventory."));
      }
    } finally {
  ...
  return {
    handleWeightSubmit: submitWeight,
    weightError,
    clearWeightError: () => setWeightError(null),
  };
}
