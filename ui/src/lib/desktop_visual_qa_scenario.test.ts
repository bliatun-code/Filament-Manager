...
test("desktop visual QA scenario parser accepts stable aliases in dev only", () => {
  ...
  assert.equal(
    normalizeDesktopVisualQaScenario("annual-consumption"),
    "dashboard-consumption",
  );
  assert.equal(
  ...
  assert.equal(
    normalizeDesktopVisualQaScenario("qr-label"),
    "selected-roll-label",
  );
  assert.equal(
  ...
  assert.equal(
    resolveDesktopVisualQaScenario("?bfm_visual_qa=loan-out", true),
    "loan-out",
  );
  assert.equal(
    resolveDesktopVisualQaScenario("?bfm_visual_qa=loan-out", false),
    null,
  );
});
...
test("desktop visual QA scenario manifest describes routing and fixture states", () => {
  ...
  assert.equal(
    desktopVisualQaScenarioDefinition("dashboard-usage")
      ?.requiresDatabaseFixture,
    true,
  );
  assert.deepEqual(desktopVisualQaScenarioDefinition("dashboard")?.readiness, {
    timeoutMs: 15_000,
    token: "dashboard-bambu-live-attention",
  });
  assert.deepEqual(
    desktopVisualQaScenarioDefinition("dashboard-usage")?.readiness,
    {
      timeoutMs: 15_000,
      token: "dashboard-consumption",
    },
  );
  assert.equal(
  ...
  assert.equal(
    desktopVisualQaScenarioDefinition("ams-onboarding")
      ?.requiresDatabaseFixture,
    true,
  );
  ...
  assert.equal(
    desktopVisualQaScenarioDefinition("printer-summary")
      ?.requiresDatabaseFixture,
    true,
  );
  assert.equal(
    desktopVisualQaScenarioDefinition("printer-summary")?.readiness,
    undefined,
  );
  assert.deepEqual(
    desktopVisualQaScenarioDefinition("add-printer")?.readiness,
    {
      timeoutMs: 15_000,
  ...
  assert.equal(
    desktopVisualQaScenarioDefinition("printer-editor")
      ?.requiresDatabaseFixture,
    undefined,
  );
  assert.equal(
    desktopVisualQaScenarioDefinition("statistics")?.page,
    "statistics",
  );
  assert.equal(
...
test("desktop annual usage screenshot data rolls with local calendar months", () => {
  const months = buildDesktopVisualQaUsageMonths(new Date(2027, 0, 15, 12));

  assert.equal(months.length, 12);
  assert.equal(months[0]?.month, "2026-02");
  assert.equal(months[11]?.month, "2027-01");
  assert.equal(
    months.reduce((total, month) => total + month.usedGrams, 0),
    3_520,
  );
  assert.ok(months.some((month) => month.usedGrams > 0));
  assert.ok(months.some((month) => month.usedGrams === 0));
});

test("desktop visual QA scenarios resolve to the page they exercise", () => {
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=dashboard-overview"),
    "dashboard",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=dashboard-onboarding"),
    "dashboard",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=dashboard-consumption"),
    "dashboard",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=inventory-overview"),
    "inventory",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=add-filament"),
    "inventory",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=wishlist-queue"),
    "inventory",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=selected-roll-history"),
    "inventory",
  );
  assert.equal(
  ...
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=rfid-capture"),
    "inventory",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=loans-overview"),
    "loans",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=return-loan"),
    "loans",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=return-inbound-loan"),
    "loans",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=printer-board"),
    "printers",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=printer-overview"),
    "printers",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=add-printer"),
    "printers",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=printer-slot-assignment"),
    "printers",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=printer-slot-onboarding"),
    "printers",
  );
  assert.equal(
  ...
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=printer-slot-replacement"),
    "printers",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=printer-slot-clear"),
    "printers",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=settings-general"),
    "settings",
  );
  assert.equal(
  ...
  assert.equal(
    desktopVisualQaInitialPage(
      "?bfm_visual_qa=settings-library-network-details",
    ),
    "settings",
  );
  assert.equal(
    desktopVisualQaInitialPage(
      "?bfm_visual_qa=settings-library-network-editor",
    ),
    "settings",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=settings-library-pairing"),
    "settings",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=settings-library-browsers"),
    "settings",
  );
  assert.equal(
    desktopVisualQaInitialPage(
      "?bfm_visual_qa=settings-library-browsers-history",
    ),
    "settings",
  );
  assert.equal(
  ...
  assert.equal(
    desktopVisualQaInitialPage(
      "?bfm_visual_qa=settings-printer-diagnostics-fields",
    ),
    "settings",
  );
  assert.equal(
    desktopVisualQaInitialPage(
      "?bfm_visual_qa=settings-printer-diagnostics-paused",
    ),
    "settings",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=settings-printer-editor"),
    "settings",
  );
  assert.equal(
  ...
  assert.equal(
    desktopVisualQaInitialPage(
      "?bfm_visual_qa=settings-printer-editor-discard",
    ),
    "settings",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=settings-catalog"),
    "settings",
  );
  assert.equal(
  ...
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=settings-maintenance"),
    "settings",
  );
  assert.equal(
    desktopVisualQaInitialPage(
      "?bfm_visual_qa=settings-application-diagnostics",
    ),
    "settings",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=statistics-overview"),
    "statistics",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=statistics-consumption"),
    "statistics",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=statistics-borrower"),
    "statistics",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=statistics-loans"),
    "statistics",
  );
  assert.equal(
    desktopVisualQaInitialPage("?bfm_visual_qa=bambu-batch-add"),
    "inventory",
  );
...
test("desktop visual QA settings scenarios resolve to the intended tab", () => {
  ...
  assert.equal(
    desktopVisualQaInitialSettingsTab(
      "?bfm_visual_qa=settings-library-role-change",
    ),
    "LIBRARY",
  );
  assert.equal(
  ...
  assert.equal(
    desktopVisualQaInitialSettingsTab(
      "?bfm_visual_qa=settings-library-pairing",
    ),
    "LIBRARY",
  );
  assert.equal(
    desktopVisualQaInitialSettingsTab(
      "?bfm_visual_qa=settings-library-browsers",
    ),
    "LIBRARY",
  );
  assert.equal(
  ...
  assert.equal(
    desktopVisualQaInitialSettingsTab(
      "?bfm_visual_qa=settings-printer-diagnostics",
    ),
    "PRINTERS",
  );
  assert.equal(
  ...
  assert.equal(
    desktopVisualQaInitialSettingsTab(
      "?bfm_visual_qa=settings-printer-editor-dirty",
    ),
    "PRINTERS",
  );
  assert.equal(
  ...
  assert.equal(
    desktopVisualQaInitialSettingsTab("?bfm_visual_qa=settings-catalog"),
    "CATALOG",
  );
  assert.equal(
    desktopVisualQaInitialSettingsTab(
      "?bfm_visual_qa=settings-catalog-swatch-review",
    ),
    "CATALOG",
  );
  assert.equal(
  ...
  assert.equal(
    desktopVisualQaInitialSettingsTab("?bfm_visual_qa=add-filament"),
    null,
  );
  assert.equal(
    desktopVisualQaInitialSettingsTab("?bfm_visual_qa=unknown"),
    null,
  );
});
...
test("desktop visual QA spool chooser prefers assigned RFID rolls for RFID capture", () => {
  ...
  assert.equal(
    chooseDesktopVisualQaSpoolId(spools, new Set(), "selected-roll"),
    "stock",
  );
});
